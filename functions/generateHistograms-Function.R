...
generate.histogramsEdges <- function (frameTable)
{
  ...
  #ggplot(frameTable, aes(x=NetworkFile)) + geom_histogram(aes(y=Edges), colour="black", fill="white", stat = "identity") + geom_density(alpha=.2, fill="#FF6666") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
  
  ggplot(data=frameTable, aes(x=NetworkFile, y=Edges)) +geom_bar(stat = "identity") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + geom_density() 
    
  histogramsEdges = paste(RESULTS_PATH, name.file,"Edges",".png", sep="");
...
generate.histogramsVertices <- function (frameTable)
{
  ...
  histogramsVertices = paste(RESULTS_PATH, name.file,"Vertices",".png", sep="");
...
generate.histogramsAverageDegree <- function (frameTable)
{
  ...
  histogramsAverageDegree = paste(RESULTS_PATH, name.file,"AverageDegree",".png", sep="");
...
generate.histogramsMaxDegree <- function (frameTable)
{
  ...
  histogramsMaxDegree = paste(RESULTS_PATH, name.file,"MaxDegree",".png", sep="");
...
generate.histogramsDiameterNetwork <- function (frameTable)
{
  ...
  histogramsDiameterNetwork = paste(RESULTS_PATH, name.file,"DiameterNetwork",".png", sep="");
...
generate.histogramsCoefficientAgroupment <- function (frameTable)
{
  ...
  histogramsCoefficientAgroupment = paste(RESULTS_PATH, name.file,"CoefficientAgroupment",".png", sep="");
...
generate.histogramsBetweennessMedium <- function (frameTable)
{
  ...
  histogramsBetweennessMedium = paste(RESULTS_PATH, name.file,"BetweennessMedium",".png", sep="");
...
generate.histogramsClosenessNetwork <- function (frameTable)
{
  ...
  histogramsClosenessNetwork = paste(RESULTS_PATH, name.file,"ClosenessNetwork",".png", sep="");
